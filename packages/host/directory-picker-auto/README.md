...
- **Detection infers operator location from launch context, which no launch-side signal can prove** — a tmux session detached from its SSH launch loses the `SSH_*` markers; a Darwin process outside an Aqua session still counts as displayed; a service-managed launch (a systemd unit, another supervisor) never carries `SSH_*` markers at all, regardless of how the operator actually connects; and a workstation-local launch later reached through `ssh -L` arrives from `127.0.0.1`, resolves `native`, and opens the chooser on the unattended workstation. A wrong `native` choice degrades to the backend's existing retryable failure dialog. Setting `DSH_DIRECTORY_PICKER_BACKEND=browse` on the launch environment (the unit or supervisor definition) selects the safe interaction for such deployments without composing `-browse` directly.
